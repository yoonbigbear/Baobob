...
namespace BaobabNodeNetwork
{
	public class Program
	{
		public static async Task Main(string[] args)
		{
			...
			await cluster.StartElection(node0);
			...
			bool success1 = await node0.SendMessage(node1, message, 1000, 0.2, 3); // 1초 지연, 20% 패킷 손실 확률, 최대 3회 재시도
			bool success2 = await node0.SendMessage(node2, message, 1000, 0.2, 3); // 1초 지연, 20% 패킷 손실 확률, 최대 3회 재시도
